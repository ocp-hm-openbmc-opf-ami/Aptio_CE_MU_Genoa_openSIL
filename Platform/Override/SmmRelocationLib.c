  ...
  )
{
  ...
  // The TSEG region overlaps with the NON SMM region when Smm_HANDLER_OFFSET is subtracted from SmmRelocationStart (Smbase).
  ...
  )
{
  ...
  //MU OE OVERRIDE START
  // Allocate SMBASE + 64KB for n-1 Processor.
  //SmmRelocationSize = EFI_PAGES_TO_SIZE (EFI_SIZE_TO_PAGES (SIZE_32KB + TileSize * (mMaxNumberOfCpus - 1)));
  SmmRelocationSize = EFI_PAGES_TO_SIZE (EFI_SIZE_TO_PAGES (SIZE_64KB + TileSize * (mMaxNumberOfCpus - 1)));
  //MU OE OVERRIDE END
