...
typedef struct _IOAPIC_LIST_ENTRY IOAPIC_LIST_ENTRY;

struct _IOAPIC_LIST_ENTRY {
  IOAPIC_LIST_ENTRY                 *ForwardLink;
  IOAPIC_LIST_ENTRY                 *BackwardLink;
  GNB_HANDLE                        *GnbHandle;
  EFI_ACPI_6_3_IO_APIC_STRUCTURE    IoApicStruct;
};

IOAPIC_LIST_ENTRY* InsertIoApicListEntry (
  IOAPIC_LIST_ENTRY           *List,
  IOAPIC_LIST_ENTRY           *NewEntry
  )
{
  IOAPIC_LIST_ENTRY           *Head;
  IOAPIC_LIST_ENTRY           *CurrentEntry;
  IOAPIC_LIST_ENTRY           *PreviousEntry;

  Head = List;
  if (Head != NULL) {
    CurrentEntry = Head;
    while (CurrentEntry != NULL) {
      if  (NewEntry->GnbHandle->Address.Address.Bus >
           CurrentEntry->GnbHandle->Address.Address.Bus) {
        if (CurrentEntry->ForwardLink != NULL) {
          CurrentEntry = CurrentEntry->ForwardLink;
          continue;
        } else {
          // Insert after
          CurrentEntry->ForwardLink = NewEntry;
          NewEntry->BackwardLink = CurrentEntry;
          goto Done;
        }
      } else {
        if (CurrentEntry == Head) {
          ...
          // Insert before
          PreviousEntry = CurrentEntry->BackwardLink;
          PreviousEntry->ForwardLink = NewEntry;
          NewEntry->BackwardLink = PreviousEntry;
        }
        NewEntry->ForwardLink = CurrentEntry;
        CurrentEntry->BackwardLink = NewEntry;
        goto Done;
      }
    }
  } else {
  ...
  )
{
  ...
  MaxMadtStructCount = (UINT32) (
    ...
    1 + TotalRootBridges + // I/O APIC structures
    // AMD_EDKII_OVERRIDE END
  ...
  if (FixedPcdGet32 (PcdMaxCpuSocketCount) <= 4) {
    ...
    MmioWrite8 (IoApicStruct.IoApicAddress + IOAPIC_INDEX_OFFSET,
              IO_APIC_VERSION_REGISTER_INDEX);
    IoApicVersionRegister.Uint32 = MmioRead32 (
      IoApicStruct.IoApicAddress + IOAPIC_DATA_OFFSET);
  ...
  while (GnbHandle != NULL) {
    IoApicListEntry = AllocateZeroPool (sizeof (IOAPIC_LIST_ENTRY));
    if (IoApicListEntry == NULL) {
      DEBUG ((DEBUG_ERROR, "Failed to allocate IoApic Structure\n"));
      goto Done;
    }
    IoApicListEntry->GnbHandle = GnbHandle;
    IoApicListEntry->IoApicStruct.Type = EFI_ACPI_6_3_IO_APIC;
  ...
  IoApicListEntry = IoApicListHead;
  // Assign APIC IDs and insert IO APIC entries in MADT
  while (IoApicListEntry != NULL) {
    IoApicIdentificationRegister.Bits.Identification += 1;
    IoApicStructPtr = &IoApicListEntry->IoApicStruct;
    // Set APIC ID
    MmioWrite8 (IoApicStructPtr->IoApicAddress + IOAPIC_INDEX_OFFSET,
                IO_APIC_IDENTIFICATION_REGISTER_INDEX);
    IoApicIdentificationRegister.Uint32 = MmioRead32 (IoApicStructPtr->IoApicAddress + IOAPIC_DATA_OFFSET);
    IoApicStructPtr->IoApicId = (UINT8)IoApicIdentificationRegister.Bits.Identification;
    // Set Global System Interrupt Base
    IoApicStructPtr->GlobalSystemInterruptBase = GlobalSystemInterruptBase;
    // Get Read the number of redirection entries in this IOAPIC
    MmioWrite8 (IoApicStructPtr->IoApicAddress + IOAPIC_INDEX_OFFSET,
                IO_APIC_VERSION_REGISTER_INDEX);
    IoApicVersionRegister.Uint32 = MmioRead32 (
        IoApicStructPtr->IoApicAddress + IOAPIC_DATA_OFFSET);
    GlobalSystemInterruptBase += IoApicVersionRegister.Bits.MaximumRedirectionEntry + 1;
    // Insert the Entry in the APCI table
    ASSERT (MadtStructsIndex < MaxMadtStructCount);
    Status = CopyStructure (
      &MadtTableHeader.Header,
      (STRUCTURE_HEADER *) IoApicStructPtr,
      &MadtStructs[MadtStructsIndex++]
      );
    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "CopyMadtStructure (I/O APIC) failed: %r\n", Status));
    ...
    IoApicListEntry = IoApicListEntry->ForwardLink;
  #if 0
  ...
  // IntSrcOverrideStruct.Flags = 0xD;                 // Flags - Level-tiggered, Active High
    IntSrcOverrideStruct.Flags = 0xF;                 // Flags - Level-tiggered, Active Low
  // AMD_EDKII_OVERRIDE END
  ASSERT (MadtStructsIndex < MaxMadtStructCount);
  Status = CopyStructure (
             &MadtTableHeader.Header,
  ...
  if (mX2ApicEnabled) {
    ...
    LocalX2ApicNmiStruct.Flags  = 0x0005;    // Flags - Edge-tiggered, Active High
    // AMD_EDKII_OVERRIDE END
  ...
  )
{
  ...
  *Version = EFI_ACPI_TABLE_VERSION_1_0B | EFI_ACPI_TABLE_VERSION_2_0 | EFI_ACPI_TABLE_VERSION_3_0;
  ...
  case EFI_ACPI_6_3_FIXED_ACPI_DESCRIPTION_TABLE_SIGNATURE:
    ...
    // AMD_EDKII_OVERRIDE START
    FadtHeader->SmiCmd             = PcdGet16 (PcdAmdFchCfgSmiCmdPortAddr);
    // AMD_EDKII_OVERRIDE END
    ...
    FadtHeader->Gpe0BlkLen = 0x8;
    if (FadtHeader->Pm2CntBlk == 0) {
      FadtHeader->Pm2CntLen = 0;
    }
    // AMD_EDKII_OVERRIDE END
    ...
    if (FadtHeader->XPm1bEvtBlk.Address == 0) {
      ...
      FadtHeader->XPm1bEvtBlk.AddressSpaceId = 0;
    }
    ...
    if (FadtHeader->XPm1bCntBlk.Address == 0) {
      ...
      FadtHeader->XPm1bCntBlk.AddressSpaceId = 0;
    }
    ...
    if (FadtHeader->XPm2CntBlk.Address == 0) {
      ...
      FadtHeader->XPm2CntBlk.AddressSpaceId = 0;
      FadtHeader->XPm2CntBlk.RegisterBitWidth = 0;
    }
    ...
    // AMD_EDKII_OVERRIDE START
    if (FadtHeader->XGpe0Blk.Address != 0) {
      FadtHeader->XGpe0Blk.RegisterBitWidth = 0x40;
    }
    // AMD_EDKII_OVERRIDE END
    ...
    // AMD_EDKII_OVERRIDE START
    FadtHeader->FlushSize = 0x400;
    FadtHeader->FlushStride = 0x10;
    FadtHeader->ResetReg.AccessSize = 0;
    FadtHeader->MinorVersion = 4;
    // AMD_EDKII_OVERRIDE END
  ...
  case EFI_ACPI_6_3_HIGH_PRECISION_EVENT_TIMER_TABLE_SIGNATURE:
    ...
    HpetTable->BaseAddressLower32Bit.RegisterBitWidth = 0x40;
    // AMD_EDKII_OVERRIDE END
