...
Cleanup:
  ...
  DEBUG ((DEBUG_INFO, "SecureBootFetchData Exit Status: %r\n", Status));
  return Status;
}

/**
  Return an EFI Signature List with data fetched from section specified as a argument.
    Process dbx in below formats
      1. ESL 
      2. AUTH2 Var
  ...
  )
{
  ...
  *SigListsSize = 0;
  ...
  if (Status == EFI_SUCCESS) {
    // 1. File -> ESL
    // check if input buffer is already formatted as EFI_SIGNATURE LIST
    // -> Append sigList and look for x509 certificates in ffs sections
    //
    EfiSig = (EFI_SIGNATURE_LIST*)Buffer;
    if( CompareGuid ((EFI_GUID*) &(EfiSig->SignatureType), &gEfiCertX509Guid) ||
        CompareGuid ((EFI_GUID*) &(EfiSig->SignatureType), &gEfiCertSha256Guid) 
        // may add more types of certificates for dbx like gEfiCertx509Sha256, etc...
    ) {  
      *SigListOut = EfiSig;
      *SigListsSize = Size;
    } else {
      // 2. File -> Efi Auth Variable
      Auth2 = (EFI_VARIABLE_AUTHENTICATION_2 *)Buffer;
      if (Auth2->AuthInfo.Hdr.wCertificateType == WIN_CERT_TYPE_EFI_GUID &&
          (CompareGuid (&gEfiCertPkcs7Guid, &Auth2->AuthInfo.CertType))
      ) {
        *SigListsSize = Size - Auth2->AuthInfo.Hdr.dwLength - sizeof (EFI_TIME);
        *SigListOut = (EFI_SIGNATURE_LIST*)AllocatePool (*SigListsSize);
        CopyMem(*SigListOut, (VOID*)((UINTN)Buffer + (Size - *SigListsSize)), *SigListsSize);
      }
      else {
        Status = EFI_INVALID_PARAMETER;
      }
  ...
  )
{
  ...
  Status = SecureBootFetchDbxData (&gDefaultdbxFileGuid, &SigListsSize, &EfiSig);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_INFO, "Content for %s not found\n", EFI_DBX_DEFAULT_VARIABLE_NAME));
    return Status;
  }
