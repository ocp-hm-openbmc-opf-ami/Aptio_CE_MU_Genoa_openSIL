...
)
{
  ...
  Status = gBS->LocateProtocol (
                  ...
                  NULL,
                  (VOID **)&mAmdPciResources
                  );
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a: Failed to locate AMD PCIe Resource Protocol: %r\n",
      __FUNCTION__,
      Status
    ));
    return Status;
  }
  Status = mAmdPciResources->AmdPciResourcesGetNumberOfRootBridges(
    mAmdPciResources,
    &NumberOfRootBridges
    );
  if (EFI_ERROR (Status)) {
  ...
  for (RBIndex = 1; RBIndex <= NumberOfRootBridges; RBIndex++) {
    ...
    Status = mAmdPciResources->AmdPciResourcesGetRootBridgeInfo(mAmdPciResources, RBIndex, &RootBridge->Object);
    if (EFI_ERROR (Status)) {
      FreePool (RootBridge);
      return Status;
    }
...
)
{
  ...
  Status = mAmdPciResources->AmdPciResourcesGetNumberOfFixedResources (
                mAmdPciResources,
  ...
  for (Index = 1; Index  <= NumberOfFixedResources; Index++) {
    Status = mAmdPciResources->AmdPciResourcesGetFixedResourceInfo (
                  mAmdPciResources,
                  RootBridge->Object->Index,
                  Index,
                  &RBFixedResource
...
)
{
  ...
  Status |= AmlName (AmlStart, "_PRT", ListHead);
    Status |= AmlPackage (AmlStart, 0, ListHead);
      ...
      Status = mAmdPciResources->AmdPciResourcesGetNumberOfRootPorts (
                  mAmdPciResources,
                  RootBridge->Object->Index,
                  &NumberOfRootPorts
                  );
      ...
      for (Index = 1; Index <= NumberOfRootPorts; Index++) {
        Status = mAmdPciResources->AmdPciResourcesGetRootPortInfo (
                    mAmdPciResources,
                    RootBridge->Object->Index,
                    Index,
                    &RootPort
...
)
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_INFO, "%a: Failed with Status: %r,\n", __FUNCTION__, Status));
    Status = EFI_DEVICE_ERROR;
  }
  ...
  Status |= AmlScope(AmlStart, "\\_SB", ListHead); // START: Scope (\_SB)
    ...
    while (Node != PciRootBridgeList) {
      ...
      Status |= AmlDevice (AmlStart, String, ListHead); // RootBridge
        ...
        Status |= InternalInsertRootPorts (RootBridge, GlobalInterruptBase, ListHead);
