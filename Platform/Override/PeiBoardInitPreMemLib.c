  ...
  )
{
  ...
  //Installing PEI memory from 0 causes Pei stack got allocated from 0 and asserts in CpuMpPei/CpuPaging.c
  Status = PeiServicesInstallPeiMemory (PcdGet32(PcdTempRamSize) + PcdGet32(PcdTempRamBase) , MemorySize);
  //MU OE OVERRIDE START
  ASSERT_EFI_ERROR(Status);
  return Status;
}
