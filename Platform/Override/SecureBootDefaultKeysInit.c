  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    Status = SetPKDefault (Data, DataSize);
  }
  ...
  )
{
  ...
  do {
    ...
    if (!EFI_ERROR (Status)) {
      Status = SetKEKDefault (Data, DataSize);
      Index++;
    }
  ...
  )
{
  ...
  do {
    ...
    if (!EFI_ERROR (Status)) {
      Status = SetSignatureDatabase (Data, DataSize, EFI_DB_DEFAULT_VARIABLE_NAME, &gEfiGlobalVariableGuid);
      Index++;
    }
  } while (Status == EFI_SUCCESS);
  ...
  )
{
  ...
  do {
    ...
    if (!EFI_ERROR (Status)) {
      Status = SetSignatureDatabase (Data, DataSize, EFI_DBX_DEFAULT_VARIABLE_NAME, &gEfiGlobalVariableGuid);
      Index++;
    }
  } while (Status == EFI_SUCCESS);
